...
import io.smallrye.asyncapi.core.runtime.scanner.dataobject.AnnotationTargetProcessor;
import io.smallrye.asyncapi.core.runtime.scanner.dataobject.AugmentedIndexView;
import io.smallrye.asyncapi.core.runtime.scanner.dataobject.DataObjectDeque;
import io.smallrye.asyncapi.core.runtime.scanner.dataobject.IgnoreResolver;
...
/**
 * Explores the class graph from the provided root, creating an AsyncAPI {@link Schema}
 ...
 *
 * @see Schema Schema Annotation
 * @see Schema Schema Object
 */
public class AsyncApiDataObjectScanner {
    ...
    private static void index(Indexer indexer, String resourceName) {
        try (InputStream stream = AsyncApiDataObjectScanner.class.getResourceAsStream(resourceName)) {
     ...
     */
    public AsyncApiDataObjectScanner(final AnnotationScannerContext context, Type classType) {
        this(context, null, classType);
    }

    public AsyncApiDataObjectScanner(final AnnotationScannerContext context, AnnotationTarget annotationTarget,
            Type classType) {
    ...
    private void depthFirstGraphSearch() {
        while (!objectStack.isEmpty()) {
            ...
            if (currentSchema.getType() == null) {
                ...
                currentSchema.setType(SchemaType.OBJECT);
            } else {
