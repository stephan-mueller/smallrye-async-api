...
public class AnnotationTargetProcessor implements BeanValidationScanner.RequirementHandler {
    ...
    private AnnotationTargetProcessor(final AnnotationScannerContext context, DataObjectDeque objectStack,
            DataObjectDeque.PathEntry parentPathEntry, TypeResolver typeResolver, AnnotationTarget annotationTarget,
            Type entityType) {
    ...
    public static Schema process(final AnnotationScannerContext context, DataObjectDeque objectStack, TypeResolver typeResolver,
    ...
    public static Schema process(final AnnotationScannerContext context, DataObjectDeque objectStack, TypeResolver typeResolver,
            DataObjectDeque.PathEntry parentPathEntry, Type type) {
        AnnotationTargetProcessor fp = new AnnotationTargetProcessor(context, objectStack, parentPathEntry, typeResolver,
                context.getAugmentedIndex()
                        .getClass(type),
                type);
        return fp.processField();
    }
    ...
    private Schema readSchemaAnnotatedField(String propertyKey, AnnotationInstance annotation, Type postProcessedField) {
        ...
        if (JandexUtil.booleanValueWithDefault(annotation, SchemaConstant.PROP_REQUIRED)) {
            parentPathEntry.getSchema()
                    .addRequired(propertyKey);
        }
    ...
    boolean fieldOverridesType(Schema fieldSchema, Schema typeSchema) {
        ...
        for (int i = 0, m = typeAttributes.size(); i < m; i++) {
            Object fieldAttr = fieldAttributes.get(i)
                    .get();

            if (fieldAttr != null) {
                Object typeAttr = typeAttributes.get(i)
                        .get();
    ...
    List<Supplier<Object>> getAttributeSuppliers(Schema schema) {
        return Arrays.asList(schema::getAdditionalPropertiesBoolean, schema::getAdditionalPropertiesSchema, schema::getAllOf,
                schema::getAnyOf, schema::getDescription, schema::getEnumeration, schema::getExample,
                schema::getExclusiveMaximum, schema::getExclusiveMinimum, schema::getExtensions, schema::getExternalDocs,
                schema::getFormat, schema::getItems, schema::getMaximum, schema::getMaxItems, schema::getMaxLength,
                schema::getMaxProperties, schema::getMinimum, schema::getMinItems, schema::getMinLength,
                schema::getMinProperties, schema::getMultipleOf, schema::getNot, schema::getOneOf, schema::getPattern,
                schema::getProperties, schema::getReadOnly, schema::getRef, schema::getRequired, schema::getTitle,
                schema::getType, schema::getUniqueItems, schema::getWriteOnly);
    }
